...
always @(negedge clk ) begin
    ...
    if(rst) begin
        ...
        if(LD_STOP)
            data_out[8] <= 1'b1;
        if(LD_ADWR)
            data_out[7:0] <= data_comd[0];
        if(LD_ADRD)
            data_out[7:0] <= data_comd[1];
        if(LD_CONF)
            data_out[7:0] <= data_comd[1+cnt_by];
        if(LD_MD)
            data_out[7:0] <= data_comd[4+cnt_by];
        if(SH_WR)
            data_out[8:0] <= {data_out[7:0],1'b0};
        if(~SDA_OE) begin
            if(SH_RD)begin
...
assign SDA = (data_out[8] == 0 && SDA_OE) ? 1'b0 : 1'bz;
assign ACK = SDA;
