...
ctrl_div ctrl_div       (.rst(rst), .clk(clk), .start(start), .mbb(w_mbb), .z(w_z), .sh(w_sh), .init(w_init), .lda(w_lda), .dv0(w_dv0), .dec(w_dec), .done(done));
...
comp_div comp_div       (.rst(rst), .in(w_comp), .z(w_z));
